...
public class ClassroomMapper {
    ...
    public static ClassroomDTO toDTO(Classroom entity) {
        ...
        return ClassroomDTO.builder()
                ...
                .teachers(Optional.ofNullable(entity.getTeachers()).orElse(new ArrayList<>()).stream().map(TeacherMapper::toDTO).collect(Collectors.toList()))
                .students(Optional.ofNullable(entity.getStudents()).orElse(new ArrayList<>()).stream().map(StudentMapper::toDTO).collect(Collectors.toList()))
    ...
    public static Classroom toEntity(ClassroomDTO dto) {
        ...
        var classroom = Classroom.builder()
                ...
                .teachers(Optional.ofNullable(dto.getTeachers()).orElse(new ArrayList<>()).stream().map(TeacherMapper::toEntity).collect(Collectors.toList()))
                .students(Optional.ofNullable(dto.getStudents()).orElse(new ArrayList<>()).stream().map(StudentMapper::toEntity).collect(Collectors.toList()))
